...
async function createDoctors() {
  ...
  const tables = db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'app_doctors'").all();
  console.log('Tables found:', tables);
  
  if (tables.length === 0) {
    console.log('❌ app_doctors table does not exist');
    return;
  }
  ...
  for (const doctor of DEMO_DOCTORS) {
    ...
    console.log(`✅ Created doctor: ${doctor.firstName} ${doctor.lastName} (${doctor.nmrUid})`);
  }

  console.log('🎉 Demo doctors created successfully!');
  console.log('📋 Demo Doctor Credentials:');
  console.log('   Username: [NMR_UID]');
  console.log('   Password: demo123');
  console.log('   Examples:');
  for (const doctor of DEMO_DOCTORS) {
    console.log(`   - ${doctor.nmrUid} / demo123 (${doctor.firstName} ${doctor.lastName} - ${doctor.specialty})`);
  }

  db.close();
}

createDoctors().catch(console.error);
